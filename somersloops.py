...
if __name__ == '__main__':
    S = int(input("How many Somersloops do you want to allocate to the recipe? "))
    assert S>=0
    m = int(input("How many Somersloop slots does one building running that recipe have? 2 or 4? "))
    assert m==2 or m==4
    I = float(input("How many times the base input rate is your available input? "))
    assert I>0
    N = int(input("How many buildings are you running the recipe in? "))
    assert N>1
    res = brute(N,I,S,m)
    print()
    if res is None:
        print("Your stated combination of sloops, input rate and buildings is impossible.")
    else:
        if res[2]:
            print(res[2], "buildings without sloops, running at", res[1]*100, "%.")
        print("below, all running at 250%:")
        if res[3]:
            print(res[3], "buildings with 1 sloop each.")
        if res[4]:
            print(res[4], "buildings with 2 sloops each.")
        if m==4:
            if res[5]:
                print(res[5], "buildings with 3 sloops each.")
            if res[6]:
                print(res[6], "buildings with 4 sloops each.")
        print("\nThe total power cost is",res[0],"times the building's base power draw.")
